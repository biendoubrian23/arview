...
### 🎨 Design / Thème (RÈGLE PRODUIT)

**Le site doit être en LIGHT MODE.**

- Fond clair (`#F7F7FB` / blanc) sur toutes les pages : landing, dashboard, viewer public, auth.
- Texte foncé (`#0F1024`) pour la lisibilité.
- Couleurs d'accent conservées : violet `#6C63FF` + bleu `#00B3D9` pour CTAs, gradients, badges.
- Pas de dark mode dans le MVP. Aucune préférence `prefers-color-scheme: dark` n'est respectée.
- Surfaces : cartes blanches avec ombre douce + bordures `rgba(15,16,36,0.08)` (au lieu du glassmorphism sombre).
...
VISUAL IDENTITY:
- Light theme (clean white / off-white background, e.g. #F7F7FB)
- Dark text on light surfaces for maximum readability (#0F1024)
