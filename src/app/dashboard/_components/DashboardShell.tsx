...
export default function DashboardShell({
  ...
  const sidebar = (
    <aside className="w-60 shrink-0 bg-bg-2 border-r border-[rgba(15,16,36,0.08)] flex flex-col h-screen sticky top-0">
      <div className="p-5">
      ...
      <div className="p-3 space-y-2 border-t border-[rgba(15,16,36,0.08)]">
        ...
        <div className="px-3 py-2 flex items-center gap-2">
          <div className="h-8 w-8 rounded-full bg-linear-to-br from-[#6C63FF] to-[#00D4FF] flex items-center justify-center text-xs font-bold text-white">
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 min-w-0 flex flex-col">
        {/* Top bar (mobile) */}
        <header className="md:hidden sticky top-0 z-40 backdrop-blur-md bg-white/80 border-b border-[rgba(15,16,36,0.08)] px-4 h-14 flex items-center gap-3">
          <button
            onClick={() => setOpen(true)}
            className="p-2 -ml-2 rounded-lg hover:bg-[rgba(15,16,36,0.04)]"
            aria-label="Menu"
          >
          ...
          {open && (
            <button
              onClick={() => setOpen(false)}
              className="ml-auto p-2 rounded-lg hover:bg-[rgba(15,16,36,0.04)]"
              aria-label="Fermer"
            >
              <X className="h-5 w-5" />
            </button>
